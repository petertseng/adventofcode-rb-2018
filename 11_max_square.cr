...
serial = Integer(!ARGV.empty? && ARGV.first.match?(/^\d+$/) ? ARGV.first : ARGF.read)

power = (0..SIZE).map { |y|
  (0..SIZE).map { |x|
    rack = x + 10
    (((rack * y + serial) * rack).digits[2] || 0) - 5
  }.freeze
}.freeze
...
puts (0..(SIZE - 3)).to_a.repeated_permutation(2).max_by { |x, y|
...
(1..SIZE).each { |sidelen|
  ...
  if sidelen > 1 && guessed_max < maxes.each_value.max[0]
    ...
    maxes[sidelen] = [guessed_max, :doomed]
    next
  end

  max_this_size = [MIN_CELL * sidelen * sidelen]
  ...
  valid.each { |ymin|
    ...
    valid.each { |xmin|
      ...
      max_this_size = [power_here, xmin + 1, ymin + 1, sidelen] if power_here > max_this_size[0]
    }
  }

  maxes[sidelen] = max_this_size.freeze
}
