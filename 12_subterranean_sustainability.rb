...
plants = input.shift.split(?:).last.strip.each_char.map { |c| c == ?# }

rules = input.drop_while(&:empty?).map { |l|
  l, r = l.split('=>').map(&:strip)
  [l.each_char.map { |c| c == ?# }.freeze, r == ?#]
}.freeze
...
gens_done = 1.step { |gen|
  plants = ([false] * 4 + plants + [false] * 4).each_cons(5).map(&rules)
