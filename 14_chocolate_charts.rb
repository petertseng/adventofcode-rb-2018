...
def find(digits)
  ...
  while true
    # Add in chunks?
    # Would be better in a language with efficient vector adds
    while first_track[first_pos + CHUNK] && second_track[second_pos + CHUNK]
      x1 = cache[first_pos] ||= (0...CHUNK).sum { |d| suffix[first_pos + d] << (5 * d) }
      x2 = cache[second_pos] ||= (0...CHUNK).sum { |d| suffix[second_pos + d] << (5 * d) }
      scores = x1 + x2
      ...
      until actual_iters == CHUNK
        return size + 1 - digits.size unless (state = state[0])
        if size == next_write
          suffix << 0
          next_write += 1
        end
    ...
    unless first_track[first_pos += 1]
      first_pos = 0
      if first_on_suffix
    ...
    unless second_track[second_pos += 1]
      second_pos = 0
      if second_on_suffix
