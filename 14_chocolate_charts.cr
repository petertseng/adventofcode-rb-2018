...
input = !ARGV.empty? && ARGV.first.match(/^\d+$/) ? ARGV.first : ""
...
unless two_only
  ...
  until scores.size >= target + 10
    scores.concat((scores[first] + scores[second]).to_s.chars.map(&.to_i8))
...
def find(digits : Array(Int8))
  ...
  next_state : StateTable = [] of StateTable
  # if using Array.new, compiler doesn't understand that it's a StateTable.
  digits.size.times { next_state << [] of StateTable }
  next_state.zip(state_transitions(digits)) { |dst, src|
    src.each { |new_good_digits|
      dst.not_nil! << next_state[new_good_digits]?
    }
  }
  state = next_state[0].not_nil!
...

{
  ...
  got = find(k.map(&.to_i8))
  puts "#{k.join}: want #{want}, got #{got}" if want != got
}

puts find(input.chars.map(&.to_i8))
